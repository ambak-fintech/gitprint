...
STATS=$(node -e "
  ...
  const repoRoot = (() => { try { return require('child_process').execSync('git rev-parse --show-toplevel', { encoding: 'utf8' }).trim(); } catch { return '$HOOK_CWD' || process.cwd(); } })();
  ...
  // If pending file had data, use only the leftover delta since post-commit
  const hasPendingData = Object.keys(pendingData).length > 0;
  let aiFiles;

  if (hasPendingData) {
    const pendingDelta = deltaFromCheckpoint(pendingData, checkpointData.files || {});
    aiFiles = Object.entries(pendingDelta).map(([file, s]) => ({
...
# ─── Clear Copilot session state ───
rm -f "$PENDING_FILE" "$ACTIVE_FILE" "$CHECKPOINT_FILE" 2>/dev/null
log "cleared Copilot pending state"
